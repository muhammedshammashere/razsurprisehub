...
export default function ProductCard({ product, onAdd }) {
  ...
  return (
    <article className="card group overflow-hidden p-0 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-900/10">
      <Link
        to={`/shop/${product._id}`}
        className="surface-gradient flex h-56 items-center justify-center overflow-hidden border-b border-brand-900/10 bg-white/80 p-3 dark:border-brand-400/15 dark:bg-slate-900/60"
      >
        <img
          src={image}
          alt={product.name}
